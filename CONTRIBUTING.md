...
korail.js 에 관심 가져 주셔서 고맙습니다. 버그 신고 한 줄부터 오타 수정까지 전부
환영합니다.
...
**TLS 지문은 아직 미해결 영역입니다.** 코레일 서버는 TLS 지문을 보는데, Node 에서
안드로이드 크롬 지문을 정확히 재현하는 검증된 수단이 없습니다. `src/transport.ts`
를 건드리는 PR 은 **실측 근거**를 함께 주세요 — 추측으로 바꾼 값은 되돌립니다.
...
**JS 예제와 TS 예제를 함께 봐 주세요.** 이 패키지는 둘 다 지원하는데, TS 로만
확인하면 `require()` 경로가 깨져도 눈에 띄지 않습니다.
...
Node 는 **20.19 이상**입니다 (20 · 22 · 24 에서 테스트합니다). 20.19 가 하한인
이유는 그 버전부터 `require(esm)` 이 되어 듀얼 패키지 위험이 크게 줄기 때문입니다.
...
실패를 `biome-ignore` · `@ts-expect-error` · 커버리지 임계값 인하로 우회하지 말고
원인을 고쳐 주세요. 정말 우회가 맞다면 **왜** 인지 같은 줄에 적어 주세요.
`@ts-ignore` 는 쓰지 마세요 — `@ts-expect-error` 는 오류가 사라지면 그 자체가
실패하므로 안전합니다.
...
```
src/
├── client.ts       Korail — 로그인·로그아웃·연결 (엔드포인트는 여기 없습니다)
├── api.ts          ApiClient — 요청/응답/서명/에러 변환
├── resources/      stations · trains · reservations · tickets  ← 엔드포인트는 전부 여기
├── models/         불변 응답 모델 + 승객·카드 파라미터 객체
├── errors/         base · api(코드매핑) · network · validation
├── device/         기기 프로파일 + 카탈로그
├── auth/           dynapath(서명) · signer · netfunnel
├── transport.ts    TLS 지문을 맞춘 HTTP 세션
└── crypto.ts       AES 원시연산 (node:crypto)
```
...
- **HTTP 는 `ApiClient` 를 통해서만.** `#session` 을 직접 만지지 마세요.
- **응답 모델은 불변** (`readonly` + `Object.freeze(this)` + `static fromResponse()`).
  객체를 만든 뒤 필드를 채우지 마세요. **`Object.freeze` 는 얕으니 배열 필드는
  복사해서 함께 동결**하세요.
- **상속이 아니라 합성.** `Ticket`·`Reservation` 은 `Train` 을 참조합니다.
- **상태 변경 메서드는 `Promise<void>`, 실패 시 throw.**
- **공개 표면은 전부 `async`.** 생성자에서 I/O 를 하지 마세요.
...
케이스가 여러 개면 `it.each` 로 펼치세요. `filter`/`map`/`some`/`every` 는 값을
뽑는 **식**이라 허용됩니다.

두 규칙 모두 `tests/style.test.ts` 가 TypeScript 컴파일러 API 로 AST 를 훑어
검사하므로, 어기면 바로 빨갛게 됩니다.

### 비동기 단언에 `await` 을 빠뜨리지 마세요

**`await` 없는 `expect(...).rejects` 는 통과합니다.** 이 저장소의 공개 API 가
전부 `async` 라 가장 흔한 사고입니다.
...
비동기 실패 경로에는 `expect.assertions(n)` 을 함께 써 주세요.

### 커버리지 90%

`vitest.config.ts` 의 `coverage.thresholds` 가 lines·statements·branches·functions
를 전부 90% 로 잡습니다. 게이트를 낮추지 말고 테스트를 써 주세요.

### 네트워크 금지

테스트는 **절대 실제 요청을 보내지 않습니다.** `tests/helpers.ts` 의 `FakeSession`
과 `korail` / `makeKorail` 픽스처를 쓰세요. 응답 샘플은 `tests/payloads.ts` 에
...
- 응답 필드 누락 · 빈 문자열 (`Number("")` 이 `0` 이 되는 경로)
- 자정을 넘기는 운행 시간
- 타임존 (KST 고정 — 실행 머신 로컬 타임존에 결과가 흔들리면 안 됩니다)
- 매진 · 예약대기 분기
- 승객 합치기 (정렬 안 된 입력)
- 로그인 실패 경로
- **듀얼 패키지 진입점** (ESM `import` / CJS `require` 양쪽)
- **모델 배열 필드를 밖에서 mutate 했을 때** (얕은 freeze 함정)
...
> [!NOTE]
> **폼 키와 응답 필드 이름은 camelCase 로 바꾸지 마세요.** `txtGoAbrdDt` ·
> `h_dpt_rs_stn_cd` 는 서버가 정한 철자입니다. JS 네이밍 관용은 **우리가 만든
> 이름**(메서드·모델 필드)에만 적용합니다.
...
커밋 요약 줄 · PR 제목 · 브랜치 이름이 **같은 접두사**를 씁니다. 한 PR 에 한 가지
변경만 담으므로 접두사도 하나로 정해집니다.

| 접두사 | 쓰는 때 |
| ------- | ------------------------------------------------------- |
| `ADD:`  | 없던 것을 새로 넣을 때 — 기능 · 엔드포인트 · 테스트 · 의존성 추가 |
| `FIX:`  | 버그 수정 — 의도한 동작과 실제가 달랐던 것 |
| `REF:`  | 코드 구조 리팩터링 — **겉보기 동작은 그대로** |
| `DEL:`  | 제거 — 기능 · 파일 · 죽은 코드 |
| `DOCS:` | 문서만 — README · TSDoc |
| `UPT:`  | 의존 패키지 **버전 업데이트** |

헷갈리기 쉬운 경계 세 가지만 미리:

- **동작이 바뀌면 `REF:` 가 아닙니다.** 새 동작이면 `ADD:`, 잘못된 동작을 바로잡은
  것이면 `FIX:`.
- **코레일이 API 를 바꿔 대응한 것은 `FIX:`** 입니다 (`external-api` 라벨도 함께).
- **의존성을 새로 넣으면 `ADD:`, 버전만 올리면 `UPT:`**.

문서·에이전트 설정은 **추가해도 `DOCS:`** 입니다 — `ADD:`·`DEL:` 은 코드와 의존성에
대한 것입니다. (`.github/workflows/` 와 `biome.json`·`tsconfig.json`·
`vitest.config.ts`·`tsup.config.ts` 는 실행·빌드 결과를 바꾸므로 코드로 봅니다.)
...
요약 줄은 한국어 명사형 종결로, 마침표 없이 50자 안팎. 본문에는 **왜** 를 적어
주세요 — **무엇**은 diff 가 이미 말합니다. 이 저장소에는 이유 있는 이상한 코드가
많아서, 다음 사람이 "정리"하려 들지 않게 하려면 이유가 남아 있어야 합니다.

형식이 틀렸다고 PR 을 돌려보내지는 않습니다. 저희가 고쳐 드릴 테니 내용에
집중하세요.

### 4. PR 제목

커밋과 같습니다 — `FIX: 과거 시각 조회를 요청 전에 차단`.

`main` 은 squash 머지가 가능해서 PR 제목이 그대로 히스토리에 남습니다. 그래서
커밋과 같은 형식을 씁니다.
...
- [ ] 네 개 게이트 전부 통과 (`pnpm verify`)
- [ ] 테스트에 `if`/`for`/`while` **문** 없음, GWT 주석 있음
- [ ] 비동기 단언에 `await` 을 빠뜨리지 않았음
- [ ] 공개 API 를 바꿨다면 `docs/reference.md` 갱신
- [ ] **외부 API 를 건드렸다면 요청 페이로드가 그대로임을 확인**
- [ ] `package.json` 의 `version` 을 건드리지 않았음 (버전은 git 태그에서 옵니다)

### 7. CI

PR 을 열면 자동으로 돕니다 — 린트 · 타입 검사 · Node 20/22/24 테스트 · 빌드 ·
**ESM/CJS 양쪽 로드 확인** · Trivy 보안 스캔.

여기에 더해 **main 으로 가는 PR** 에는 **CodeRabbit** 이 리뷰 코멘트를 답니다
(설정은 [`.coderabbit.yaml`](.coderabbit.yaml)). 초안(draft) PR 은 건너뜁니다.
필수 통과 항목이 아니니 지적에 동의하지 않으시면 그렇게 답해 주세요 — 사람이
판단합니다. PR 에서 `@coderabbitai` 로 말을 걸 수도 있습니다.

> 다만 **리뷰 스레드는 전부 resolve 해야 머지됩니다** (아래 8번). 반영하든 안
> 하든 답글을 달고 해결 처리해 주세요.
...
태그를 밀면 버전 주입 → 전 버전 게이트 → Trivy → 빌드 → **ESM/CJS 로드 확인** →
npm 업로드(provenance 포함) → 릴리스 노트 생성이 자동으로 돕니다.
